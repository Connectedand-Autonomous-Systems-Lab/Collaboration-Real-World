...
def generate_launch_description():
    ...
    rviz_node = Node(
        ...
        name='rviz2',
        output='screen',
        arguments=['-d', 'src/hololens_ros2_bridge/rviz/oakd.rviz', '--ros-args', '--log-level', 'fatal'],
    ...
    slam_toolbox = IncludeLaunchDescription(
        ...
        launch_arguments={
            'use_sim_time': 'true',
            'slam_params_file': os.path.join(get_package_share_directory('hololens_ros2_bridge'), 'config', 'oakd.yaml'),
        }.items()
    )   
